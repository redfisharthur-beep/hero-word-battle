...
export class GameRoom extends DurableObject<Env> {
  ...
  private snapshot(s: RoomState): Snapshot {
    return new Map(s.players.map((p) => [p.id, { hp: p.hp, maxHp: p.maxHp, atk: p.atk, def: p.def, guard: p.guard, alive: p.alive }]))
  }
  ...
  private changesFrom(before: Snapshot, s: RoomState): StatChange[] {
    ...
    for (const p of s.players) {
      ...
      if (change.hp !== undefined || change.maxHp !== undefined || change.atk !== undefined || change.def !== undefined || change.guard !== undefined || change.ko) changes.push(change)
    }
    return changes
  }
  ...
  private nextResolveStep(s: RoomState) {
    ...
    if ((s.resolveIndex ?? 0) >= (s.resolveQueue?.length ?? 0)) { this.finishRound(s); return }
  ...
  async alarm() { const s=await this.getState();const a=this.pruneStale(s),b=this.advanceTimeouts(s);if(a||b||s.players.length)await this.save(s) }
